...
function ConnectWalletButton({ buttonClassName, dropdownClassName, dropdownItemClassName, wallets = index_js_1.walletConfig, }) {
    ...
    return (react_1.default.createElement("div", { className: "z-50" }, !connected ? (react_1.default.createElement(DropdownMenu.Root, null,
        react_1.default.createElement(DropdownMenu.Trigger, { asChild: true },
            react_1.default.createElement("button", { type: "button", className: (0, style_js_1.cn)("cursor-pointer py-2.5 min-w-fit px-4 border border-primary text-nowrap text-primary hover:scale-105 rounded-lg font-medium flex items-center justify-center gap-2 transition-all duration-300", buttonClassName) },
        ...
        react_1.default.createElement(DropdownMenu.Portal, null,
            react_1.default.createElement(DropdownMenu.Content, { className: (0, style_js_1.cn)("z-50 min-w-[12rem] overflow-hidden rounded-xl border border-primary bg-light p-1 shadow-md bg-primary", dropdownClassName), sideOffset: 5 }, Object.entries(wallets).map(([key, { label, icon }], index) => (react_1.default.createElement(DropdownMenu.Item, { key: key, onClick: async () => await connectWallet(key), className: (0, style_js_1.cn)(`flex items-center px-4 py-3 text-sm font-medium cursor-pointer hover:bg-primary hover:border-none hover:text-light text-primary transition-all duration-300 ease-in-out transform hover:scale-105 ${index < Object.entries(wallets).length - 1 ? "border-b border-primary" : ""}`, dropdownItemClassName) },
