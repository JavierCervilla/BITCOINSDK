...
exports.UTXOBuyOrderAction = void 0;
...
function UTXOBuyOrderAction({ order }) {
    const [loading, setLoading] = (0, react_2.useState)(false);
    const { signPSBT, walletAddress } = (0, index_js_2.useWallet)();
    async function sendTransaction() {
        ...
        console.log({ params });
        const { psbt, inputsToSign } = await index_js_1.default.openbook.getBuyOrderPSBT(params);
    ...
    return (react_1.default.createElement("button", { className: "w-full ml-10 sm:ml-2 cursor-pointer text-sm text-primary rounded-lg p-2 bg-warning border border-primary hover:bg-primary hover:text-light transition-colors font-mono truncate", type: "button", onClick: handleBuy }, "Buy"));
}
exports.UTXOBuyOrderAction = UTXOBuyOrderAction;
