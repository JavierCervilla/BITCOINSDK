...
import { HamburgerMenu } from "@/components/HamburgerMenu/HamburgerMenu.component.tsx"
import type { MarketData } from "@/types/openbook.d.ts"
import { cn } from "@/lib/utils/style.ts";


const CryptoInfo: React.FC<{ data: MarketData }> = ({ data }) => {
...
export function Header() {
  ...
  const fetchBalances = useCallback(async (address: string) => {
    const [xcpData, btcData] = await Promise.all([
      counterparty.getTokenBalance({ asset: 'XCP', address }),
      openbook.getBTCBalance({ address })
    ])
    setBalances({
      BTC: btcData,
  ...
  useEffect(() => {
    openbook.getMarketData().then((data) => {
      setCryptoData(data)
    })
  }, [])
    ...
    <div className="w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="flex max-w-full min-w-fit flex-col lg:flex-row items-center justify-between px-4 py-3 lg:px-6 gap-4 mx-auto">
        <div className="flex flex-col w-full sm:flex-row gap-4 p-3 rounded-lg border border-border/50">
          {cryptoData.map((crypto: MarketData, index: number) => (
            <div
              key={crypto.id}
